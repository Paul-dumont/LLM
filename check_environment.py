...
def check_file_permissions():
    ...
    print_status("Vérification des permissions", "INFO")
    
    # Test création dossiers
    test_dirs = ["runs", "Data_predict", "logs"]
    
    for dir_name in test_dirs:
        try:
            Path(dir_name).mkdir(exist_ok=True)
            print_status(f"Permissions {dir_name}: OK", "OK")
        except PermissionError:
            print_status(f"Permissions {dir_name}: ÉCHEC", "ERROR")
            return False
    ...
    try:
        ...
        test_file.unlink()
        print_status("Permissions écriture: OK", "OK")
    except Exception as e:
